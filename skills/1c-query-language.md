...
description: Как правильно писать запросы на языке 1С для получения данных из базы: структура ВЫБРАТЬ/ИЗ/ГДЕ, имена таблиц, виртуальные таблицы (Остатки/Обороты/ОстаткиИОбороты/СрезПоследних, регистр бухгалтерии со Счётом и Субконто), соединения, временные таблицы, параметры и типовые паттерны.
---

<!-- Источник: Desko77/claude-code-skills-1c (MIT), адаптировано для 1С Ассистента. См. SKILLS-NOTICE.txt -->
...
Before composing a query, if you are not certain about the exact names, **use `get_structure/describe` or any other available tool to retrieve 1C configuration metadata** to discover:
...
Every 1C metadata object has a fixed query-name pattern. Use exact names from `get_structure/describe`.
...
For index-aware optimization strategy, see optimization-and-pitfalls.md.
